...
void handleRoot(WiFiClient &client) {
  ...
  <head>
    ...
    <title>)" + String(BW_TITLE_STR) + R"( - WebUI</title>
    <style>
  ...
  <body>
  <div class="container">
    <h1 class="bold">)" + String(BW_TITLE_STR) + R"(</h1>
      <div class="right">
  ...
  + R"(
      ...
      <div class="right")" + String(!isDeauthing ? " style=\"display: none\"" : "") + R"(>
        <form method="post" action="/stop">
          <div class="button-container">
            <input type="submit" value="Stop" class="attack-stop">
          </div>
        </form>
      </div>
...
void web_stable(){
  ...
  if (client.connected()){
    ...
    if (path == "/") {
      ...
      scanNetworks();
    } else if (path == "/perform_attack") {
      std::vector<std::pair<String, String>> post_data = parsePost(request);
      int attack_mode = 0;

      // Check which attack button was selected
      for (auto& param : post_data) {
